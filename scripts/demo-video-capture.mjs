...
const sceneHoldMs = Number(process.env.DEMO_VIDEO_SCENE_HOLD_MS || 4500);
const transitionPauseMs = Number(process.env.DEMO_VIDEO_TRANSITION_MS || 600);
...
const scenes = [
  ...
  {
    id: "01b-dashboard-empty",
    title: "Trang chủ người mới",
    path: "/",
    caption: "Bản đồ ghim chu kỳ cho người mới",
    scroll: 0.3,
  },
  {
    id: "02-onboarding",
  ...
  },
  {
    id: "12-vision-board",
    title: "Vision Board",
    path: "/vision-board/demo_video_board",
    caption: "Thiết kế Bảng tầm nhìn mơ ước",
    scroll: 0.3,
    requiresWorkspace: true,
  },
  {
    id: "13-order",
    title: "Đặt bộ Kit vật lý",
    path: "/order",
    caption: "Đặt bộ công cụ hành động vật lý",
    scroll: 0.3,
    requiresWorkspace: true,
  },
  {
    ...
    path: "/billing/plan",
    caption: "Nâng cấp gói tài khoản Plus nâng cao",
    scroll: 0.3,
    requiresWorkspace: true,
  },
];
...
Đặc biệt, hệ thống tích hợp Bảng tầm nhìn số hóa sinh động giúp bạn trực quan hóa những ước mơ của mình với hình ảnh và trích dẫn truyền cảm hứng.

Hơn thế nữa, bạn có thể dễ dàng đặt mua Bộ Kit hành động vật lý - in ấn cao cấp để ghim lên tường làm việc, mang mục tiêu từ màn hình ra thế giới thực.

Toàn bộ trải nghiệm được thiết kế local-first để không mất tiến độ khi mạng chập chờn, đồng thời production có đăng nhập, đồng bộ bảo mật, thanh toán Plus tự động và quản lý tài khoản chuyên nghiệp.
...
function createDemoSeedForScene(sceneId, uid = "demo_video_user") {
  const now = new Date();
  ...
  const onboardingCompleted = sceneId !== "02-onboarding" && sceneId !== "01b-dashboard-empty";
  const wheelOfLife = (sceneId === "02-onboarding" || sceneId === "01b-dashboard-empty") ? [] : currentWheelOfLife;
  ...
  const userData = {
    storageVersion: 8,
    userId: uid,
    wheelOfLifeHistory: onboardingCompleted
      ? [
          {
      ...
      ? [
          {
            ...
            items: [
              ...
              },
              {
                id: "board_item_goal",
                type: "goal_card",
                content: "Gửi 20 hồ sơ ứng tuyển chất lượng trong 12 tuần",
                x: 80,
                y: 260,
                width: 320,
                height: 120,
    ...
    aspirationalVision: {
      ...
      horizonYears: 3,
      summary:
        "Trở thành một professional có portfolio rõ ràng, nhịp làm việc bền vững và lựa chọn nghề nghiệp tốt hơn.",
      lifeAreas: onboardingCompleted
        ? [
            {
              area: "career",
  ...
  const storage = {
    ...
    pending_12_week_setup_draft: [
      ...
      ? JSON.stringify(pending12WeekSetupDraft)
      : "",
    latest_12_week_goal_id: hasActiveGoals ? goalId : "",
    latest_12_week_system_goal_id: hasActiveGoals ? goalId : "",
    latest_12_week_plan_goal_id: hasActiveGoals ? goalId : "",
...
async function scrollForScene(page, ratio) {
  const scrollRatio = typeof ratio === "number" ? ratio : 0.35;
  ...
  await page.evaluate(async (targetRatio) => {
    ...
    const targetY = Math.round(maxScroll * targetRatio);

    if (targetY <= 0) return;

    const duration = 3800; // 3.8s cuộn cực kỳ thong thả, mượt mà và cinematic
    ...
    await new Promise((resolve) => {
      function step(now) {
        ...
        window.scrollTo(0, Math.round(targetY * ease));
...
async function main() {
  ...
  try {
    let loggedInUid = hasCredentials ? null : "demo_video_user";
    ...
    if (hasCredentials) {
      ...
      const initialSeed = createDemoSeedForScene("01-opening", loggedInUid);
      await seedBrowserStorage(page, initialSeed, true);
    }

    for (let index = 1; index < scenes.length; index += 1) {
      const scene = scenes[index];
      log(`Chuẩn bị seed dữ liệu cho cảnh: ${scene.title} (${scene.id})...`);
      const sceneSeed = createDemoSeedForScene(scene.id, loggedInUid);
      await seedBrowserStorage(page, sceneSeed, false);
      capturedScenes.push(await captureScene(page, scene, index));
    }
