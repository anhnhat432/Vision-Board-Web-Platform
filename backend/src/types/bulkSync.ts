import type { TaskStatus } from "../repositories/mongo/MongoTaskRepository";
...
export interface BulkSyncTaskInput {
  /** Remote task ID — omit or empty for "create new" */
  taskId?: string;
  weekId: string;
  title: string;
  status: TaskStatus;
...
export interface BulkSyncTaskResult {
  /** The local/temp ID the client sent (taskId field) — echoed back for linking */
  clientTaskId?: string;
  taskId: string;
  weekId: string;
  ok: boolean;
