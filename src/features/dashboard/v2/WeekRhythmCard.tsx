...
export function WeekRhythmCard({
  ...
  return (
    <section data-testid="dashboard-kpi-row" className="rounded-card border border-app-line bg-app-surface p-5 md:p-6" aria-labelledby="dashboard-week-rhythm-title">
      <div>
        <h2 id="dashboard-week-rhythm-title" className="text-[15px] font-semibold text-app-ink">
          Nhịp tuần {safeWeek}
        </h2>
        ...
        {stats.map((item) => {
          ...
          return (
            <div key={item.caption} className="rounded-xl border border-app-line bg-app-bg p-3">
              ...
              <p className="mt-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">{item.caption}</p>
              <p className="mt-1 font-serif text-[26px] font-medium leading-none text-app-ink sm:text-[28px]">{item.value}</p>
              <p className="mt-2 text-[11px] text-app-ink-muted">{item.subLine}</p>
            </div>
          );
        })}
      </div>
