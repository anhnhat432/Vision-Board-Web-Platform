    ...
    >
      <div>
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-app-ink-muted">{caption}</p>
      ...
      <div data-tour-id="dashboard-plan-card" className="hidden rounded-card border border-app-line bg-app-surface p-5 md:block">
        <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-app-ink-muted">Mục tiêu nổi bật</p>
        <p className="mt-2 line-clamp-2 text-[14px] font-medium leading-5 text-app-ink">{featuredGoalTitle}</p>
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className="text-[12px] font-semibold text-app-accent">{safeProgress}%</span>
        </div>
        <Link
          to={planHref}
          className="mt-4 inline-flex text-[13px] font-medium text-app-accent transition-colors duration-150 hover:text-app-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30"
        >
          Mở kế hoạch tuần →
        </Link>
      </div>
