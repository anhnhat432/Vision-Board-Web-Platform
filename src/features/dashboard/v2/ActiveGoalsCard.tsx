  ...
  return (
    <section className="rounded-card border border-app-line bg-app-surface p-5 md:p-6" aria-labelledby="dashboard-active-goals-title">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 id="dashboard-active-goals-title" className="text-[15px] font-semibold text-app-ink">
            Mục tiêu đang chạy
          </h2>
        ...
        <button
          ...
          onClick={onAddGoal}
          disabled={isAtLimit}
          className="inline-flex items-center justify-center gap-1.5 rounded-full border border-app-line bg-app-surface px-3 py-1.5 text-[13px] font-medium text-app-accent transition-colors duration-150 hover:bg-app-accent-soft disabled:cursor-not-allowed disabled:text-app-ink-muted disabled:hover:bg-app-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30"
      ...
      <div className="mt-5">
        {visibleGoals.length > 0 ? (
            ...
            return (
              <article key={goal.id} className="flex gap-4 border-b border-app-line py-4 last:border-0">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="line-clamp-2 text-[14px] font-medium leading-5 text-app-ink">{goal.title}</h3>
                ...
                <div className="flex shrink-0 flex-col items-end justify-between gap-3 text-right">
                  <span className="text-[12px] font-semibold tabular-nums text-app-ink">{progress}%</span>
                  <button
                    type="button"
                    onClick={() => onSelectGoal(goal)}
                    className="inline-flex items-center gap-1 text-[13px] font-medium text-app-accent transition-colors duration-150 hover:text-app-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30"
          ...
          <div className="rounded-card border border-dashed border-app-line bg-app-bg p-5 text-[14px] leading-6 text-app-ink-soft">
