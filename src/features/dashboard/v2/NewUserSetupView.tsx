...
const getStepTheme = (index: number): CardStepTheme => {
  ...
  return {
    border: "border-app-line dark:border-app-line",
    hoverBg: "hover:bg-app-bg-subtle/50 dark:hover:bg-app-bg-subtle/40 hover:border-app-accent/30",
    iconBgPending: "bg-app-bg-subtle dark:bg-app-bg-subtle",
...
export function NewUserSetupView({ userData, displayName, onContinue }: NewUserSetupViewProps) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto w-full">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-app-line dark:border-app-line bg-gradient-to-br from-app-accent-subtle via-app-surface to-app-accent-soft/40 dark:from-app-accent-subtle/30 dark:via-app-bg-subtle/40 dark:to-app-bg p-6 md:p-10 shadow-3xs">
        ...
        <div className="relative z-10 space-y-4 max-w-3xl">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-app-accent/20 bg-app-accent-soft dark:border-app-accent/20 px-3 py-0.5 text-[9px] font-bold uppercase tracking-wide text-app-accent dark:text-app-accent shadow-sm">
          ...
          <p className="text-xs sm:text-sm font-medium leading-relaxed text-app-ink-soft dark:text-app-ink-muted font-serif italic max-w-2xl">
            Biến mục tiêu lớn thành kế hoạch 12 tuần và việc cần làm mỗi ngày, để bạn biết bắt đầu từ đâu.
          </p>

          <div className="pt-2 flex items-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-app-line dark:border-app-line bg-app-surface/90 dark:bg-app-surface/90 px-3.5 py-1 text-[11px] font-semibold text-app-ink-soft dark:text-app-ink-muted shadow-sm">
      ...
      <section
        ...
        className="rounded-3xl border border-app-line dark:border-app-line bg-app-surface/40 p-6 md:p-8 shadow-3xs w-full"
        aria-labelledby="dashboard-new-user-title"
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-app-line dark:border-app-line/60 pb-6 mb-8">
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-app-accent hover:bg-app-accent-hover px-6 py-2.5 text-xs font-bold text-white shadow-md transition-all duration-200 hover:-translate-y-px active:translate-y-0 active:scale-[0.99] cursor-pointer"
          >
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 grid gap-4 relative select-none">
              ...
              return (
                <button
                  ...
                  className={`group flex text-left gap-4 rounded-2xl border p-5 transition-all duration-300 active:scale-[0.99] cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30 relative ${
                    step.completed
                      ? "border-app-line bg-app-bg-subtle/20 dark:bg-app-bg-subtle/5 opacity-55 hover:opacity-85 hover:border-app-line-strong"
                      : isNextStep
                        ? "bg-app-surface dark:bg-app-surface border-app-accent/65 shadow-[0_8px_30px_rgba(47,93,80,0.06)] ring-1 ring-app-accent/15"
                        : `bg-app-surface/40 dark:bg-app-surface/20 ${theme.border} ${theme.hoverBg} ${theme.rotate}`
                  }`}
                >
                  {/* Index / Check bubble */}
                      ...
                      className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
                        step.completed
                          ...
                          : isNextStep
                            ...
                            : `border border-app-line-strong dark:border-app-line-strong bg-app-surface dark:bg-app-surface text-app-ink-muted`
                      }`}
                    >
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5 pt-0.5">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                      <h3
                        className={`text-xs font-bold leading-none ${step.completed ? "text-app-ink-muted line-through" : "text-app-ink dark:text-app-ink"}`}
                      >
                        {step.title}
                      </h3>
                      ...
                      <span
                        className={`inline-block rounded-full px-2 py-0.5 text-[9px] font-bold tracking-wide ${
                          isNextStep && !step.completed
                            ...
                            : "bg-app-bg-subtle dark:bg-app-bg-subtle text-app-ink-muted"
                        }`}
                      >
                        {theme.badgeText}
                      </span>
                      ...
                      {isNextStep && !step.completed && (
                        <span className="inline-block rounded-full bg-app-accent-soft text-app-accent dark:text-app-accent px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide border border-app-accent-soft/50">
                          Đề xuất
                        </span>
                      )}
                    </div>
                    ...
                    <p
                      className={`text-xs font-semibold leading-relaxed ${step.completed ? "text-app-ink-muted" : "text-app-ink-muted dark:text-app-ink-muted"}`}
                    >
                      {step.description}
                    </p>
                  </div>
            ...
            <div className="mt-4 rounded-2xl border border-app-line dark:border-app-line bg-app-surface/30 dark:bg-app-surface/20 shadow-3xs overflow-hidden">
              <button
                type="button"
                onClick={() => setShowSamplePlan(!showSamplePlan)}
                className="w-full flex items-center justify-between px-5 py-4 text-xs font-bold text-app-ink-soft dark:text-app-ink-muted hover:bg-app-bg-subtle/50 dark:hover:bg-app-bg-subtle/40 transition-colors cursor-pointer select-none"
              >
              ...
              {showSamplePlan && (
                <div className="px-5 pb-5 border-t border-app-line dark:border-app-line pt-4 bg-app-surface/30 overflow-hidden">
                  <DreamToPlanPreview />
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            {hasLifeBalance && userData.currentWheelOfLife && userData.currentWheelOfLife.length > 0 ? (
              <div className="rounded-3xl border border-app-line dark:border-app-line bg-app-surface/60 dark:bg-app-surface/20 p-5 shadow-xs relative">
                  ...
                  {userData.currentWheelOfLife.map((area) => (
                    <div key={area.name} className="space-y-1">
                      <div className="flex justify-between items-center text-xs font-bold">
                        <span className="text-app-ink-soft dark:text-app-ink-muted">{translateArea(area.name)}</span>
                        <span className="text-app-accent">{area.score}/10</span>
                      </div>
                      <div
                        className="h-1.5 overflow-hidden rounded-full bg-app-bg-subtle dark:bg-app-bg-subtle"
                        aria-hidden="true"
                      >
                        <div
              ...
              <div className="space-y-4">
                <div className="relative rounded-3xl border border-app-line dark:border-app-line overflow-hidden shadow-3xs aspect-square w-full group select-none transition-all duration-300 hover:shadow-md">
                  <img
                ...
                <div className="rounded-2xl border border-dashed border-app-line dark:border-app-line p-5 text-center space-y-2.5 bg-app-surface/30">
                  <Target className="h-5 w-5 mx-auto text-app-accent" />
                  <h4 className="text-xs font-bold text-app-ink dark:text-app-ink">
                    Định hình cuộc sống bạn muốn
                  </h4>
                  <p className="text-[10px] leading-relaxed text-app-ink-muted dark:text-app-ink-muted font-semibold">
            ...
            <div className="relative rounded-3xl border border-app-line dark:border-app-line overflow-hidden shadow-3xs aspect-video w-full group select-none">
              <img
