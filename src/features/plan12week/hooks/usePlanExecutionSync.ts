...
export function usePlanExecutionSync(options: UsePlanExecutionSyncOptions) {
  ...
  const getPlanDeduped = useCallback(
    (planId: string): Promise<PlanDetails> => {
  ...
  const runAction = useCallback(
    async <T>(
      action: () => Promise<T>,
      conflictMeta?: { entityType: ConflictInfo["entityType"]; entityId: string },
    ): Promise<T | null> => {
      if (isDemoMode()) {
  ...
  const ensurePlanDetails = useCallback(
    async (goalId: string, system: TwelveWeekSystem): Promise<PlanDetails | null> => {
  ...
  const ensureRemoteMetricId = useCallback(
    async (goalId: string, weekId: string, weekNumber: number, metricName: string): Promise<string | null> => {
  ...
  // Sync daily check-in
  const syncDailyCheckInForWeek = useCallback(
    async (goalId: string, weekId: string, input: SyncDailyCheckInInput): Promise<boolean> => {
      ...
      if (metrics) {
        ...
        if (metric) {
          ...
          const existingLogForDate = [...metric.logs]
            .reverse()
            .find((log) => inputDateKey && getNormalizedMetricLogDateKey(log.date) === inputDateKey);
  ...
  const syncCompletedTaskMetricForWeek = useCallback(
    async (goalId: string, weekId: string, task: TwelveWeekTaskInstance): Promise<boolean> => {
  ...
  // Sync weekly review
  const syncWeeklyReviewForWeek = useCallback(
    async (weekId: string, input: SyncWeeklyReviewInput): Promise<boolean> => {
  ...
  const syncTaskToggleNow = useCallback(
  ...
  const syncDailyCheckInNow = useCallback(
  ...
  const syncWeeklyReviewNow = useCallback(
  ...
  const applyBulkSyncResponse = useCallback(
    (currentGoalId: string, systemToSync: TwelveWeekSystem, response: BulkSyncResponse, counter: SyncCounter): void => {
      // Weeks
        ...
        if (metricResult.ok) {
          if (metricResult.metricId) {
            ...
            if (weekNumber >= 0) {
              setMetricIdForGoal(
                currentGoalId,
                systemToSync.weeklyPlans[weekNumber].weekNumber,
  ...
  const syncPlanSnapshotLegacy = useCallback(
    async (
      ...
      for (const task of systemToSync.taskInstances) {
        ...
        const weekId =
          getWeekIdForGoal(currentGoalId, task.weekNumber) ?? findWeekDetails(details, task.weekNumber)?.id;
      ...
      for (const review of systemToSync.weeklyReviews.filter(hasReviewContent)) {
        const weekId =
          getWeekIdForGoal(currentGoalId, review.weekNumber) ?? findWeekDetails(details, review.weekNumber)?.id;
        if (!weekId) {
          counter.skippedCount += 1;
          continue;
        }
  ...
  const syncPlanSnapshotNow = useCallback(
    async (systemToSync: TwelveWeekSystem | null | undefined): Promise<PlanExecutionSyncSnapshot> => {
    ...
    [goalId, ensurePlanDetails, applyBulkSyncResponse, syncPlanSnapshotLegacy],
  );

  // Internal executeSync function for queue processor
  const executeSyncInternal = useCallback(
    async (item: SyncQueueItem): Promise<unknown> => {
    ...
    [system, syncTaskToggleNow, syncDailyCheckInNow, syncWeeklyReviewNow, syncPlanSnapshotNow],
  );
  ...
  const syncLocalSnapshot = useCallback(
    async (input: SyncLocalSnapshotInput = {}): Promise<PlanExecutionSyncSnapshot> => {
