    ...
    <div className="mx-auto max-w-4xl space-y-5">
      <section className="rounded-card border border-app-line bg-app-surface p-5 sm:p-6" aria-labelledby="schedule-main-title">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p id="schedule-main-title" className="text-[14px] font-medium text-app-ink">
      ...
      <section className="rounded-lg border border-app-line bg-app-bg p-3" aria-labelledby="schedule-summary-title">
        <p id="schedule-summary-title" className="text-[13px] font-medium text-app-ink">
          Chu kỳ 12 tuần
        </p>
        <p className="mt-1 text-[13px] leading-6 text-app-ink-soft">
      ...
      {setupGuideSupport && setupGuideTemplate ? (
        <section className="rounded-lg border border-app-line bg-app-bg p-3" aria-labelledby="schedule-guide-title">
          <p id="schedule-guide-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">
          ...
          <div className="mt-3 rounded-lg border border-app-line bg-app-surface p-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Gợi ý duy trì</p>
            <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{setupGuideSupport.week1CadenceHint}</p>
          </div>
        </section>
      ) : null}
      ...
      {setupGuideSupport ? (
        <section className="rounded-lg border border-app-line bg-app-bg p-3" aria-labelledby="schedule-recommendation-title">
          <p id="schedule-recommendation-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">
            Ngày nhìn lại và mức tải tuần gợi ý
          </p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-app-line bg-app-surface p-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Nhìn lại</p>
              <p className="mt-2 text-[13px] font-medium text-app-ink">{draft.reviewDay}</p>
              <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{setupGuideSupport.recommendedReviewReason}</p>
            </div>
            <div className="rounded-lg border border-app-line bg-app-surface p-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Nhịp tuần</p>
              <p className="mt-2 text-[13px] font-medium text-app-ink">
                {getLoadPreferenceLabel(draft.tacticLoadPreference)}
              </p>
              <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{setupGuideSupport.recommendedLoadReason}</p>
            </div>
          </div>
        </section>
      ) : null}
      ...
      {(draft.week4Milestone || draft.week8Milestone) && (
        <section className="rounded-lg border border-app-line bg-app-bg p-3" aria-labelledby="schedule-milestones-title">
          <p id="schedule-milestones-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={milestone.label} className="rounded-lg border border-app-line bg-app-surface p-3">
                ...
                <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{milestone.value}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="rounded-lg border border-app-line bg-app-bg p-3" aria-labelledby="schedule-week-one-title">
        <p id="schedule-week-one-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">
        ...
        <div className="mt-3 space-y-2">
          {weekOneTaskPreview.length === 0 ? (
            ...
            weekOneTaskPreview.map((task) => (
              <div key={task} className="rounded-lg border border-app-line bg-app-surface px-4 py-3 text-[13px] text-app-ink-soft">
                {task}
              </div>
            ))
          )}
