...
const infoBoxClass = "rounded-lg border border-app-line bg-app-bg p-3 text-[12px] leading-5 text-app-ink-soft";
const chipClass =
  "rounded-full border border-app-line px-3 py-1 text-[12px] text-app-ink-soft transition-colors duration-150 hover:border-app-accent hover:bg-app-accent-soft hover:text-app-accent";
    ...
    <div className="mx-auto max-w-4xl space-y-5">
          ...
          <ul className="mt-3 grid gap-2 md:grid-cols-2">
            {planRationaleReasons.map((reason) => (
              <li key={reason.id} className="rounded-lg border border-app-line bg-app-surface p-3">
                <p className="text-[13px] font-medium text-app-ink">{reason.title}</p>
                <p className="mt-1 text-[12px] leading-5 text-app-ink-soft">{reason.detail}</p>
              </li>
            ))}
          </ul>
          {feasibility.smartGoalQualityNote ? (
            <div className="mt-3 rounded-lg border border-app-line bg-app-surface px-3 py-2 text-[12px] leading-5 text-app-ink-soft">
              {feasibility.smartGoalQualityNote}
            </div>
          ) : null}
        </details>
      ...
      {(feasibility.bottleneck || feasibility.firstWeekGuidance || feasibility.scopeRecommendation) && (
        <div className="grid gap-3 md:grid-cols-3">
          <div className={infoBoxClass}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Cần chú ý</p>
            <p className="mt-1 text-[13px] font-medium text-app-ink">{feasibility.bottleneck?.label ?? "Chưa có"}</p>
            {feasibility.bottleneck?.action ? <p className="mt-2">{feasibility.bottleneck.action}</p> : null}
          </div>
          <div className={infoBoxClass}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Tuần 1</p>
            <p className="mt-1">{feasibility.firstWeekGuidance ?? "Giữ tuần đầu vừa sức để tạo nhịp."}</p>
          </div>
          <div className={infoBoxClass}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Mức tải</p>
            <p className="mt-1 text-[13px] font-medium text-app-ink">{getPlanLoadLabel(feasibility.planLoad)}</p>
      ...
      <section className="rounded-card border border-app-line bg-app-surface p-5 sm:p-6" aria-labelledby="outcome-required-title">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p id="outcome-required-title" className="text-[14px] font-medium text-app-ink">
          ...
          <span className="rounded-full border border-app-line bg-app-bg px-3 py-1 text-[12px] font-medium text-app-ink-muted">
            Bắt buộc
          </span>
        </div>
      ...
      {selectedTemplate ? (
        <section className="rounded-card border border-app-line bg-app-surface p-5" aria-labelledby="template-personalize-title">
          <div>
            <p id="template-personalize-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-accent">
              Cá nhân hóa khung
            </p>
            <p className="mt-1 text-[13px] leading-6 text-app-ink-soft">
                ...
                {WEEKDAY_LABELS.map((dayLabel, dayIndex) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "rounded-md border border-app-line bg-app-surface px-2 py-2 text-[13px] text-app-ink-soft transition-colors duration-150 hover:bg-app-bg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30",
      ...
      <section className="rounded-card border border-app-line bg-app-surface p-5" aria-labelledby="template-picker-title">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p id="template-picker-title" className="text-[14px] font-medium text-app-ink">
        ...
        {recommendedTemplate && adaptiveTemplateRecommendation ? (
          <div className="mt-4 rounded-lg border border-app-line bg-app-bg p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-accent">Gợi ý cho mục tiêu này</p>
                <p className="mt-2 text-[16px] font-medium text-app-ink">{recommendedTemplate.name}</p>
                <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{adaptiveTemplateRecommendation.reason}</p>
              </div>
              <span className="rounded-full border border-app-line bg-app-surface px-3 py-1 text-[12px] text-app-ink-muted">
            ...
            <button
              type="button"
              className={cn(
                "mt-4 inline-flex items-center justify-center rounded-lg px-4 py-2 text-[14px] font-medium transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/30",
            ...
            {recommendedTemplateSupport ? (
              <details className="mt-4 rounded-lg border border-app-line bg-app-surface px-4 py-3">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <div className={infoBoxClass}>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Tuần 1 nên thắng ở đâu</p>
                    <p className="mt-2 text-[13px] font-medium text-app-ink">{recommendedTemplateSupport.week1Headline}</p>
                    <p className="mt-2">{recommendedTemplateSupport.week1Support}</p>
                  </div>
                  <div className={infoBoxClass}>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Nhịp nên giữ</p>
        ...
        <details className="mt-4 rounded-lg border border-dashed border-app-line bg-app-bg px-4 py-3">
          <summary className="cursor-pointer list-none text-[13px] font-medium text-app-ink">Xem tất cả khung mẫu</summary>
            ...
            {TWELVE_WEEK_TEMPLATE_CATALOG.map((template) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-[14px] font-medium">{template.name}</p>
                        <span className="rounded-full border border-app-line bg-app-surface px-2.5 py-1 text-[11px] text-app-ink-muted">
                          {template.requiredPlan ? `Khung ${getPlanLabel(template.requiredPlan)}` : "Khung miễn phí"}
                        </span>
                      </div>
                      <p className="mt-1 text-[13px] leading-5 text-app-ink-soft">{template.subtitle}</p>
                    </div>
                    <span className="inline-flex items-center gap-1 rounded-full border border-app-line bg-app-surface px-2.5 py-1 text-[11px] text-app-ink-muted">
                      {isLocked ? <Lock className="h-3 w-3" aria-hidden="true" /> : null}
                      {isSelected ? "Đang dùng" : isLocked ? "Đang khóa" : "Sẵn sàng"}
                    </span>
                  </div>
                  <p className="mt-3 text-[13px] leading-6 text-app-ink-soft">{template.description}</p>
                  <div className="mt-3 grid gap-2 text-[12px] leading-5 text-app-ink-soft sm:grid-cols-2">
                    <div className="rounded-lg border border-app-line bg-app-surface px-3 py-2">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Hợp khi</p>
                  ...
                  {isLocked ? (
                    <div className="mt-4 flex items-center justify-between border-t border-app-line pt-3 text-[12px] font-medium text-app-accent">
      ...
      {selectedTemplate ? (
        <section className="rounded-card border border-app-line bg-app-surface p-4" aria-labelledby="selected-template-title">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p id="selected-template-title" className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">
                Khung đang dùng
              </p>
              <p className="mt-2 text-[15px] font-medium text-app-ink">{selectedTemplate.name}</p>
              <p className="mt-1 text-[13px] leading-5 text-app-ink-soft">{selectedTemplate.subtitle}</p>
            </div>
            <span className="rounded-full border border-app-line bg-app-bg px-3 py-1 text-[12px] text-app-ink-muted">
      ...
      <details className="rounded-lg border border-dashed border-app-line bg-app-bg p-4">
        <summary className="cursor-pointer list-none text-[13px] font-medium text-app-ink">Xem mục tiêu đã viết</summary>
        <div className="mt-4 grid gap-3">
          <div className="rounded-lg border border-app-line bg-app-surface p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Mục tiêu cụ thể</p>
            <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{smartGoal.specific}</p>
          </div>
          <div className="rounded-lg border border-app-line bg-app-surface p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-app-ink-muted">Cách đo kết quả</p>
            <p className="mt-2 text-[13px] leading-6 text-app-ink-soft">{smartGoal.measurable}</p>
          </div>
        </div>
      </details>
    </div>
