    ...
    <div className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between border-b border-app-line pb-3">
        ...
        <button
          ...
          className="inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-app-accent text-white hover:brightness-105 disabled:opacity-50 transition-all px-3.5 py-1.5 text-xs font-bold shrink-0 focus-visible:ring-2 focus-visible:ring-app-accent focus-visible:ring-offset-2 focus:outline-none"
        >
      ...
      <div className="rounded-2xl border border-indigo-100 dark:border-indigo-950/50 bg-indigo-50/30 dark:bg-indigo-950/10 p-4 text-xs text-app-ink-soft flex gap-3 items-start select-none">
        ...
        <div className="space-y-1">
          ...
          <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 pt-1.5">
            <div className="text-[11px] leading-relaxed text-rose-600 dark:text-rose-400 font-medium">
              ❌ <strong>Tránh ghi (Kết quả):</strong> Giảm 5kg, đọc hết 10 cuốn sách, đạt IELTS 7.0...
            </div>
            <div className="text-[11px] leading-relaxed text-emerald-600 dark:text-emerald-400 font-medium">
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-app-line/40 pb-2">
              ...
              {draft.leadIndicators.length > 2 && (
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[10px] font-bold text-app-ink-muted hover:text-red-500 transition-colors py-0.5 rounded focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-1 focus:outline-none"
                >
            ...
            <div className="grid gap-3.5 sm:grid-cols-12">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {currentSuggestions.map((suggestion) => (
                      <button
                        ...
                        className="inline-flex rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 px-2 py-0.5 text-[9px] font-semibold text-app-ink-soft hover:border-indigo-300 hover:bg-indigo-50/20 active:scale-95 transition-all focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-1 focus:outline-none"
                      >
                        {suggestion}
                      </button>
                    ))}
              ...
              <div className="sm:col-span-3 space-y-1.5">
                ...
                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/60 border border-app-line rounded-xl p-1.5 w-full max-w-[140px] shadow-sm">
                  <button
                    type="button"
                    onClick={() => {
                      soundService.click();
                ...
                <div className="flex flex-wrap gap-1 pt-0.5">
                  {QUICK_UNITS.map((u) => (
                    <button
                      ...
                      className={cn(
                        "inline-flex rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-[9px] font-bold transition-colors active:scale-90 focus-visible:ring-2 focus-visible:ring-app-accent focus-visible:ring-offset-1 focus:outline-none",
                        indicator.unit === u ? "bg-app-accent text-white" : "text-app-ink-soft hover:bg-slate-200",
                      )}
                    >
                      {u}
                    </button>
            ...
            <div className="mt-4 rounded-xl border border-app-line bg-app-bg/40 p-3.5 space-y-2">
              <button
                type="button"
                ...
                className="flex w-full items-center justify-between text-xs font-bold text-indigo-650 dark:text-indigo-400 py-1 px-2 rounded hover:bg-indigo-50/50 transition-all select-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-1 focus:outline-none"
              >
      ...
      {weekOneTaskWarning && (
        <div className="rounded-xl border border-amber-300 bg-amber-50/60 dark:bg-amber-950/20 p-3.5 flex gap-2.5 shadow-3xs animate-in slide-in-from-bottom-2 duration-200">
          <AlertTriangle className="h-4.5 w-4.5 text-amber-600 dark:text-amber-500 shrink-0 mt-0.5" />
          <div>
            <span className="font-extrabold text-[9px] uppercase tracking-wider text-amber-800 dark:text-amber-400 block mb-0.5">
              Lời khuyên về tải trọng hành động:
            </span>
            <p className="text-xs font-semibold text-amber-950 dark:text-amber-300 leading-relaxed">
              {weekOneTaskWarning}
            </p>
          </div>
        </div>
