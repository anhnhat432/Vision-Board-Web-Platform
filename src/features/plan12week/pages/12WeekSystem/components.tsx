...
export function TwelveWeekDashboardState({
  ...
  return (
    <div className="mx-auto mt-12 max-w-2xl rounded-card border border-app-line bg-app-surface p-8 text-center">
      ...
      <p className="mt-4 text-xs font-medium uppercase tracking-[0.18em] text-app-ink-muted">{eyebrow}</p>
      <h1 className="mt-2 font-serif text-2xl font-medium leading-tight text-app-ink">{title}</h1>
      <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-app-ink-soft" role="status">
        {description}
      </p>
      {children}
    </div>
    ...
    <div role={role} className={`rounded-card border p-4 ${toneClass}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm leading-6 text-app-ink-soft">{description}</p>
        </div>
