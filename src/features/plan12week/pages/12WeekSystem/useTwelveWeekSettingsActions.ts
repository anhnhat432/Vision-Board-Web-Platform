...
export function useTwelveWeekSettingsActions({
  ...
  const handleReviewDayChange = useCallback(
    (value: string) => {
      if (!system) return;
  ...
  const handleLoadPreferenceChange = useCallback(
    (value: string) => {
      if (!system) return;
      commitPlanSnapshotUpdate({
        ...system,
  ...
  const handleTacticPriorityChange = useCallback(
    (tacticId: string | undefined, value: string) => {
  ...
  const handleTacticTypeChange = useCallback(
    (tacticId: string | undefined, value: string) => {
      if (!activeGoal || !system || !tacticId) return;
  ...
  const handleTimeBlocksChange = useCallback(
    (blocks: TimeBlock[]) => {
      if (!system) return;
  ...
  const handleOutboxItemToggle = useCallback(
    (item: SyncOutboxItem) => {
  ...
  const handleExportCloudWorkspace = async () => {
    if (isDemoMode()) {
      toast.info(
        "Chế độ chỉ lưu trên thiết bị chưa bật xuất dữ liệu tài khoản. Hãy dùng bản xuất dữ liệu trên thiết bị.",
      );
      return;
    }
    try {
  ...
  const handleDeleteAllData = useCallback(async () => {
    ...
    try {
      ...
      if (accountDeleteResult && !accountDeleteResult.firebaseAccountDeleted) {
        ...
        toast.success(
          shouldDeleteRemoteAccount ? "Đã xóa tài khoản và dữ liệu." : "Đã xóa toàn bộ dữ liệu trên thiết bị.",
        );
      }
