...
export function calculatePlanInsights(plan: Plan12Week | null): PlanInsights {
  ...
  const averageScore = Math.round(weekScores.reduce((sum, item) => sum + item.score, 0) / weekScores.length);

  const bestWeek = weekScores.reduce((best, current) => (current.score > best.score ? current : best));

  const worstWeek = weekScores.reduce((worst, current) => (current.score < worst.score ? current : worst));

  const variance = weekScores.reduce((sum, item) => sum + (item.score - averageScore) ** 2, 0) / weekScores.length;
