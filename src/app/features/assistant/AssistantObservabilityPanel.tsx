...
export function AssistantObservabilityPanel({ userId, onClose }: ObservabilityPanelProps) {
  ...
  const [isClearEventsOpen, setIsClearEventsOpen] = useState(false);
  ...
  return (
    <>
      <div className="flex flex-col h-full bg-slate-950 text-slate-100 border-l border-slate-800 w-full max-w-2xl overflow-hidden font-sans">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setIsClearEventsOpen(true)}
              title="Xóa log sự kiện"
              aria-label="Xóa log sự kiện"
            ...
            <div className="space-y-6">
              ...
              <div className="text-center p-4 rounded-lg bg-indigo-950/10 border border-indigo-900/10">
                <p className="text-xs text-slate-400 italic">
                  💡 Hệ thống lưu trữ các tương tác cục bộ để phân tích chất lượng phản hồi, tự động redacted dữ liệu
                  nhạy cảm.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                  Chạy kiểm thử chất lượng AI assistant dựa trên {EVAL_CASES.length} trường hợp chuẩn bị sẵn, đánh giá
                  khả năng phản hồi văn bản, loại bỏ rò rỉ secrets, gọi action tương thích và hỏi làm rõ
                  (clarification).
                </p>
      ...
      <AlertDialog open={isClearEventsOpen} onOpenChange={setIsClearEventsOpen}>
        <AlertDialogContent className="surface-elevated rounded-2xl border border-slate-700 bg-slate-950 text-slate-100 shadow-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-serif text-slate-50">Xóa toàn bộ event history?</AlertDialogTitle>
            <AlertDialogDescription className="text-slate-300">
              Hành động này sẽ xóa toàn bộ log observability của Trợ lý AI trên thiết bị này. Dữ liệu chat và mục tiêu
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              className="bg-red-600 text-white hover:bg-red-500 animate-none duration-0"
              onClick={handleClear}
            >
              Xóa event history
            </AlertDialogAction>
