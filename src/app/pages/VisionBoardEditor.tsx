...
export function VisionBoardEditor() {
  ...
  const currentPlanForPaywall = useMemo(
    () => getCurrentPlan(isVisionBoardLimitPaywallOpen ? getUserData() : editorUserData),
  ...
  useEffect(() => {
    if (id) {
      const existingBoard = editorUserData.visionBoards.find((item) => item.id === id);
      if (existingBoard) {
  ...
  const availableGoalCards = useMemo(() => {
    if (!board) return editorGoals;
    const pinnedGoalIds = new Set(board.items.filter((item) => item.type === "goal_card").map((item) => item.content));
    return editorGoals.filter((goal) => !pinnedGoalIds.has(goal.id));
  }, [board, editorGoals]);

  const goalCardProgressById = useMemo(
    () => Object.fromEntries(editorGoals.map((goal) => [goal.id, calculateGoalProgress(goal)])),
    [editorGoals],
  );
  ...
  const handleAddGoalCard = () => {
    ...
    const goal = editorGoals.find((item) => item.id === selectedGoalId);
    if (!goal) {
  ...
  const handleUpdateItem = useCallback((itemId: string, updates: Partial<VisionBoardItem>) => {
    setBoard((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
  ...
  return (
    <div
      className="stack-section mx-auto max-w-7xl px-4 pb-12 pt-6 sm:px-6 lg:px-8"
    ...
    >
      <UpgradePaywallDialog
        ...
        context="plan"
        currentPlan={currentPlanForPaywall}
          ...
          <CardContent className="relative p-6 sm:p-7 lg:p-8">
            <div className="relative z-10 grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_312px]">
              <div className="flex flex-col gap-5">
                <span
                  className="inline-flex items-center gap-[8px] self-start rounded-full bg-app-accent-subtle px-[13px] py-[6px] text-[10px] font-extrabold uppercase tracking-[0.14em] text-app-accent whitespace-nowrap"
                ...
                <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_96px]">
                  <Input
                    ...
                    className="h-[46px] rounded-control border-app-line/20 bg-app-bg-subtle dark:bg-app-bg-subtle text-[13.5px] font-medium"
                  />
                  <Input
                    type="number"
                    placeholder="Năm"
                    ...
                    className="h-[46px] rounded-control border-app-line/20 bg-app-bg-subtle dark:bg-app-bg-subtle text-center font-mono text-[13.5px] font-semibold"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  <Button
                    onClick={handleSave}
                    className="bg-app-accent text-white hover:bg-app-accent-hover rounded-[11px] text-[13px] font-bold"
                  >
                ...
                {!boardName.trim() ? (
                  <p className="text-[11.5px] text-app-ink-muted">
                    ...
                    <span className="font-semibold text-app-ink-soft">Lưu bảng</span>.
                  </p>
                ) : null}
                ...
                <div className="flex flex-wrap items-center gap-3 border-t border-app-line/10 pt-5">
                      ...
                      <button
                        ...
                        style={{
                          ...
                          borderColor:
                            themeId === theme.id
                              ? "#fff"
                              : theme.preview.gradient.includes("#F7F4ED")
                                ? "rgba(23,21,15,0.14)"
                                : "transparent",
                          boxShadow: themeId === theme.id ? "0 0 0 2px #fff, 0 0 0 4px var(--app-accent)" : "none",
                        }}
                  ...
                  <button
                    ...
                    className={`ml-auto inline-flex items-center gap-2 rounded-full border px-3 py-2 text-[11.5px] font-bold transition-all duration-150 ${
                      showZones
                        ? "border-[rgba(12,94,58,0.25)] bg-app-accent-subtle text-app-accent dark:border-app-accent/25 dark:bg-app-accent-soft dark:text-app-accent"
                        : "border-[rgba(23,21,15,0.12)] bg-white text-app-ink-soft dark:border-app-line dark:bg-app-surface dark:text-app-ink-soft"
                    }`}
                  >
                    <span
                      className={`h-[7px] w-[7px] rounded-full ${showZones ? "bg-app-accent dark:bg-app-accent" : "bg-[#C7C2B5] dark:bg-app-ink-muted"}`}
                    />
              ...
              <div className="hidden flex-col gap-4 xl:flex">
                ...
                <div className="flex flex-col gap-3">
                  <div className="rounded-[13px] border border-app-line/10 dark:border-app-line bg-app-bg-subtle dark:bg-app-bg-subtle px-4 py-4 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-app-ink-muted">
                        Tổng phần tử
                      </p>
                      <p className="text-[11.5px] text-app-ink-soft">đang có trên bảng</p>
                    </div>
                    <span className="font-serif text-[30px] font-extrabold leading-none text-app-ink">
                      {board.items.length}
                    </span>
                  </div>
                  <div className="rounded-[13px] border border-app-line/10 dark:border-app-line bg-app-bg-subtle dark:bg-app-bg-subtle px-4 py-4 flex items-center justify-between gap-3">
                    ...
                    <span className="font-serif text-[30px] font-extrabold leading-none text-app-accent">
                      {boardStats.images}
                    </span>
                  </div>
                  <div className="rounded-[13px] border border-app-line/10 dark:border-app-line bg-app-bg-subtle dark:bg-app-bg-subtle px-4 py-4 flex items-center justify-between gap-3">
                    ...
                    <span className="font-serif text-[30px] font-extrabold leading-none text-app-ink">
                      {boardStats.quotes + boardStats.icons}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-app-line mt-2 animate-in fade-in duration-200">
                      <div className="stack-tight">
                        <div className="flex items-center gap-2 text-sm font-semibold text-app-ink">
                          ...
                          Gắn ảnh vào vùng nào?{" "}
                          <span className="text-xs font-normal text-app-ink-soft">(tùy chọn)</span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {LIFE_AREAS.map((area) => (
                            <button
                ...
                emptyStateSlot={
                  board.items.length === 0 ? (
                    <div className="absolute inset-0 flex items-center justify-center p-4 sm:p-8">
                      <div className="relative z-10 w-full max-w-[380px] rounded-card dark:border-app-line bg-white dark:bg-app-surface px-9 py-10 text-center shadow-[0_30px_60px_-30px_rgba(0,0,0,0.5)] dark:shadow-[0_30px_60px_-30px_rgba(0,0,0,0.8)]">
