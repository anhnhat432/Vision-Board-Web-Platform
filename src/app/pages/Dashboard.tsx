...
const DASHBOARD_TOUR_STEPS: SpotlightTourStep[] = [
  {
    ...
    title: "B?t d?u t? kh?i này",
    description:
      "N?u chua có chu k?, hãy nhìn kh?i này tru?c. Ðây là noi d?n b?n qua dúng lu?ng: góc nhìn, m?c tiêu SMART, ki?m tra tính th?c t? r?i m?i vào 12 tu?n.",
  },
  {
    id: "attention",
    targetId: "dashboard-next-card",
    title: "Nhìn kh?i này tru?c khi quét c? màn",
    description: "Ph?n d?u Trang chính gom ba tín hi?u quan tr?ng nh?t d? b?n bi?t nên m? vào dâu ti?p theo.",
  },
  {
    id: "plan",
    targetId: "dashboard-plan-card",
    title: "Phân bi?t Free và Plus ? dây",
    description: "Kh?i này cho bi?t b?n dang ? gói nào, quy?n nào dã m? và ch? d? qu?n lý ho?c khôi ph?c l?i n?u c?n.",
  },
];
...
function getDashboardGreeting(now = new Date()): string {
  ...
  if (hour >= 5 && hour <= 11) return "Chào bu?i sáng";
  if (hour >= 12 && hour <= 17) return "Chào bu?i chi?u";
...
function getLastSavedLabel(userData: UserData, tasks: TwelveWeekTaskInstance[]): string {
  ...
  if (minutes < 60) return `${minutes} phút tru?c`;

  const hours = Math.round(minutes / 60);
  if (hours < 24) return `${hours} gi? tru?c`;

  return `${Math.round(hours / 24)} ngày tru?c`;
}
...
export function Dashboard() {
  ...
  useSetAssistantPageContext({
    pageType: "dashboard",
    hint: "Ðang xem t?ng quan m?c tiêu và h? th?ng 12 tu?n",
  });
...
function useDashboardDerivedData({
  ...
  const dashboardGoalTitle = visibleActiveTwelveWeekGoal?.title ?? plan?.vision ?? "M?c tiêu hi?n t?i";
  ...
  // Preview uu tiên task hôm nay; n?u tr?ng thì fallback sang task tu?n này
  // chua hoàn thành. Khi fallback, counter cung d?i sang ngu?n tu?n d?
  // "0/0 vi?c" không hi?n sai cùng 3 task render t? tu?n.
  ...
  const todayPreviewTitle = todayPreviewUsesToday ? "Vi?c hôm nay" : "Vi?c tu?n này";
  ...
  return (
    ...
    >
      ...
      <SpotlightTour
        ...
        description="Ba di?m chính d? ngu?i m?i m? vào là bi?t nên b?t d?u t? dâu."
...
function FreeGoalLimitCard({ current, limit, onUpgrade }: { current: number; limit: number; onUpgrade: () => void }) {
  return (
    <section className="mb-5 surface-raised rounded-xl border border-app-line bg-app-surface p-4" aria-label="Gi?i h?n gói Free">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-app-ink">
            Gói Free: {current}/{limit} m?c tiêu
          </p>
          <p className="mt-1 text-sm leading-6 text-app-ink-muted">
            Nâng c?p Plus khi b?n c?n t?o thêm m?c tiêu m?i. D? li?u cu v?n du?c gi? nguyên.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <span>
          <span className="font-semibold text-app-ink">
            {demoMode ? "Plus dùng th?:" : "Plus dang trong th?i gian uu dãi:"}
          </span>{" "}
          còn {daysLeft} ngày {demoMode ? "trên trình duy?t này" : "trên tài kho?n này"}.
        </span>
        <button
          type="button"
          onClick={onOpenPlan}
  ...
  if (planLoading && !hasPlan) {
    return (
      <div className="surface-raised rounded-xl border border-app-line bg-app-surface p-4 text-sm text-app-ink-muted">
        Ðang t?i d? li?u Trang chính 12 tu?n...
      </div>
    );
  }
...
function DemoDataNotice({ onOpenLifeBalance }: { onOpenLifeBalance: () => void }) {
  return (
    <section className="mt-6 rounded-card border border-app-line bg-app-accent-soft p-5" aria-label="D? li?u m?u">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-app-accent">D? li?u dang hi?n th? là ví d? m?u</p>
          <p className="mt-1 text-sm leading-6 text-app-ink-soft">
            C?p nh?t bánh xe cu?c s?ng c?a b?n d? thay d? li?u m?u b?ng thông tin th?t.
          </p>
        </div>
        <button
          type="button"
