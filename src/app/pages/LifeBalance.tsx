...
export function LifeBalance() {
  ...
  const handleSave = () => {
    saveLifeBalance();
    toast.success("Ðã c?p nh?t Cân b?ng cu?c s?ng", {
      description: "Ði?m m?i dã du?c luu vào bánh xe cu?c s?ng c?a b?n.",
    });
  };
        ...
        <div ref={pageTopRef} className="pb-12">
          <header>
            ...
            <h1 className="mt-3 font-serif text-4xl font-medium leading-tight tracking-tight text-app-ink">
              B?c tranh hi?n t?i c?a b?n
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-app-ink-soft">
              Nhìn 8 linh v?c d? bi?t b?n dang m?nh ? dâu, m?ng ? dâu.
            </p>
          </header>
          ...
          <section className="mt-8 surface-empty rounded-2xl border border-dashed border-app-line bg-app-bg/50 p-8 text-center">
            ...
            <h2 className="mt-4 font-serif text-2xl font-medium text-app-ink">Chua có d? li?u bánh xe</h2>
            <p className="mx-auto mt-2 max-w-md text-sm text-app-ink-soft">
              B?t d?u b?ng cách ch?m di?m 8 linh v?c d? xem b?c tranh.
            </p>
            <Link
              to="/onboarding"
              className="mt-6 inline-flex items-center gap-2 rounded-lg bg-app-accent px-5 py-2.5 text-sm font-medium text-white hover:bg-app-accent-hover"
            >
              B?t d?u ch?m di?m
    ...
    <PageShell maxWidth="xl">
      <AlertDialog open={blocker.state === "blocked"}>
        <AlertDialogContent className="surface-elevated rounded-2xl border border-app-line bg-app-surface shadow-[var(--shadow-3)]">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="font-serif text-app-ink">B?n có thay d?i chua luu</AlertDialogTitle>
            <AlertDialogDescription className="text-sm text-app-ink-soft">
              N?u r?i kh?i trang này, h? th?ng s? c? luu di?m v?a ch?nh tru?c khi dóng trang.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-col gap-3 sm:flex-col sm:items-stretch sm:justify-start">
            <AlertDialogAction
              onClick={() => {
                saveLifeBalance();
                toast.success("Ðã luu tru?c khi r?i trang.");
                blocker.proceed?.();
              }}
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-app-accent px-4 py-2 text-sm font-medium text-white hover:bg-app-accent-hover"
            >
      ...
      <div ref={pageTopRef} className="pb-12">
        <header className="page-enter">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-app-ink-muted">Bánh xe cu?c s?ng</p>
          <h1 className="mt-3 font-serif text-4xl font-medium leading-tight tracking-tight text-app-ink">
            B?c tranh hi?n t?i c?a b?n
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-app-ink-soft">
        ...
        <Tabs defaultValue="current" className="mt-8">
          <TabsList>
            <TabsTrigger value="current">Hi?n t?i</TabsTrigger>
          ...
          <TabsContent value="current" className="mt-6">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <section className="surface-raised rounded-xl border border-app-line bg-app-surface p-5 md:p-6">
                <header>
                  <h2 className="text-base font-semibold text-app-ink">Bánh xe cu?c d?i</h2>
                  <p className="text-xs text-app-ink-muted">8 linh v?c hi?n t?i</p>
                </header>
                <div className="mt-4">
              ...
              <section className="surface-raised rounded-xl border border-app-line bg-app-surface p-5 md:p-6">
                <header>
                  <h2 className="text-base font-semibold text-app-ink">C?p nh?t di?m</h2>
                  <p className="text-xs text-app-ink-muted">Kéo thanh d? ch?m l?i</p>
                </header>
                <ul className="mt-5">
                    ...
                    return (
                      <li key={area.name} className="border-b border-app-line py-3 last:border-0">
                        ...
                        <Slider
                          ...
                          max={10}
                          step={1}
                          aria-label={`Ði?m ${getLifeAreaLabel(area.name)}`}
                        />
                      </li>
                    );
                  })}
                ...
                <div className="mt-5 flex justify-end">
                  <button
                    ...
                    className={
                      hasChanges
                        ? "inline-flex items-center gap-2 rounded-lg bg-app-accent px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-app-accent-hover"
          ...
          <TabsContent value="history" className="mt-6">
            <section className="surface-raised rounded-xl border border-app-line bg-app-surface p-5 md:p-6">
              <header>
                <h2 className="text-base font-semibold text-app-ink">Di?n bi?n 6 d?t g?n nh?t</h2>
                <p className="text-xs text-app-ink-muted">M?i du?ng là m?t linh v?c; tr?c d?ng là di?m 0–10.</p>
              </header>
              <div className="mt-4">
                {historicalData.length > 0 ? (
                  ...
                  <div className="rounded-card border border-dashed border-app-line p-8 text-center">
                    <p className="text-sm text-app-ink-soft">Chua có l?ch s?. Hãy luu thay d?i d? xem di?n bi?n.</p>
                  </div>
                )}
              </div>
            </section>
