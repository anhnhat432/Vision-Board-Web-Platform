...
export function SMARTGoalSetup() {
  ...
  if (!firstStepData) {
    throw new Error("Các bu?c m?c tiêu SMART chua du?c c?u hình.");
  }
  ...
  useSetAssistantPageContext({
    ...
    hint:
      currentStepKey === "specific"
        ? "Ðang vi?t m?c tiêu c? th?, có th? hình dung du?c"
        : currentStepKey === "measurable"
          ? "Ðang d?t thu?c do d?nh lu?ng cho m?c tiêu"
          : currentStepKey === "achievable"
            ? "Ðang ki?m tra m?c tiêu có làm du?c trong di?u ki?n hi?n t?i"
            : currentStepKey === "relevant"
              ? "Ðang xác d?nh lý do m?c tiêu này quan tr?ng"
              : currentStepKey === "timeBound"
                ? "Ðang ch?t deadline và timeline"
                : undefined,
  });
  ...
  const summaryRows = useMemo(
    () =>
      SMART_STEPS.map((step) => ({
        key: step.key,
        label: step.label,
        value: formatStepDraft(step.key, smartData) || "Chua có n?i dung cho ph?n này.",
      })),
    [smartData],
  );
    ...
    return (
      <PageShell maxWidth="md">
        <div className="space-y-6">
          ...
          <FormSkeleton aria-label="Ðang chu?n b? bu?c vi?t m?c tiêu" />
        </div>
      </PageShell>
    );
  }
  ...
  if (setupState === "needs_life_balance") {
    return (
      <CoreFlowGateState
        ...
        eyebrow="Vi?t m?c tiêu"
        title="Hoàn thành bu?c cân b?ng tru?c"
        description="Ch?m di?m các linh v?c cu?c s?ng tru?c d? m?c tiêu d?a trên d? li?u th?t, không ph?i s? m?c d?nh."
        actionLabel="B?t d?u cân b?ng"
  ...
  if (setupState === "needs_life_insight") {
    return (
      <CoreFlowGateState
        ...
        eyebrow="Vi?t m?c tiêu"
        title="Ch?n tr?ng tâm tru?c"
        description="Ðã có d? li?u cân b?ng nhung chua ch?n tr?ng tâm. Ch?n m?t linh v?c r?i quay l?i vi?t m?c tiêu."
        actionLabel="M? bu?c ch?n tr?ng tâm"
  ...
  return (
    <PageShell maxWidth="md">
      <UpgradePaywallDialog
        ...
        title="B?n dã có 3 m?c tiêu"
        description="Nâng c?p Plus d? t?o thêm m?c tiêu. D? li?u hi?n có v?n du?c gi? nguyên."
          ...
          >
            ...
            {aspirationalVision ? (
              <p className="mt-3 font-serif text-lg font-medium leading-7 text-app-warm-strong">
                M?c tiêu này ph?c v? t?m nhìn 3 nam: {aspirationalVision.summary}
              </p>
            ) : (
              <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm leading-6 text-app-ink-soft">
                  B?n dang d?t m?c tiêu 12 tu?n. Hãy nghi thêm v? t?m nhìn 3 nam tru?c.
                </p>
              ...
              {SMART_STEPS.map((step) => (
                <div key={step.key} className="rounded-lg border border-app-line bg-app-surface p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-app-ink">
                    {formatStepDraft(step.key, smartData) || "Chua có n?i dung cho ph?n này."}
                  </p>
                </div>
              ))}
            </div>
