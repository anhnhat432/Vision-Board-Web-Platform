...
export function WeeklyReviewIllustration({ className, ...rest }: AmbientIllustrationProps) {
  ...
  return (
    ...
    >
      ...
      {WEEK_CELLS.map((cell) => (
        <rect key={cell.id} x={cell.x} y="112" width="17" height="44" rx="6" fill="white" fillOpacity={cell.fillOpacity} />
      ))}
      {CHECKMARKS.map((mark) => (
        <path key={mark.id} d={`M${mark.x} 134L${mark.x + 5} 140L${mark.x + 13} 126`} stroke="currentColor" strokeOpacity="0.58" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      ))}
