...
- Production API 429 root cause was middleware composition, not only quota size: root-mounted `healthRateLimiter` and `authProfileRateLimiter` counted unrelated API traffic, while `generalApiRateLimiter` pooled all authenticated domains. The fix isolates named route-family quotas, bounds planning hydration/retry, removes duplicate automatic legacy sync when mutation sync is enabled, and keeps unrecovered 429 responses fatal in production smoke. Focused local verification is recorded on the fix branch; full repository checks and deployed smoke evidence remain pending.
...
- Account-delete and LWW account secrets are configured, and all four proof workflows are available on the default branch. Live `npm run proof:secrets` on 2026-07-30 reports the shared Automation Bypass secret missing by name; protected-preview runs remain blocked until it is configured, followed by successful deployed proof runs and evidence recording.
- Manual core-flow launch evidence is still pending on the protected demo preview. Local preflight passed on 2026-06-26 and was re-verified on 2026-06-27. Workflow run `28899920950` on 2026-07-07 failed because it targeted the production real-mode domain, and a 2026-07-08 preview recheck stopped at Vercel login. The next run keeps Deployment Protection enabled and uses `VERCEL_AUTOMATION_BYPASS_SECRET`; no public preview is required.
