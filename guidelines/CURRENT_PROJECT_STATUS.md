...
- The historical production smoke blocker from run `28995039420` is closed. The verified release proof in section 5.1 records four staging passes and two consecutive production smoke passes on one final `main` SHA.
- Production API 429 root cause was middleware composition, not only quota size: root-mounted `healthRateLimiter` and `authProfileRateLimiter` counted unrelated API traffic, while `generalApiRateLimiter` pooled all authenticated domains. The merged fix isolates named route-family quotas, bounds planning hydration/retry, removes duplicate automatic legacy sync when mutation sync is enabled, and keeps unrecovered 429 responses fatal in production smoke.
...
- Deployed core-funnel proof workflow now exists at `.github/workflows/core-funnel-quality-staging.yml`. It runs the same local-first `npm run smoke:core-quality` script against the protected demo preview (`VITE_APP_MODE=demo`) with the GitHub Automation Bypass secret; Vercel Deployment Protection remains enabled. Playwright proofs use in-memory bypass headers with traces disabled, while `agent-browser` uses an owner-only temporary config removed after the run. The workflow still rejects `localhost`, `127.0.0.1`, and the production real-mode domain.
...
- `npm run proof:secrets` now performs the GitHub secret-name readiness audit locally without reading secret values and fails when required staging proof secrets, including `VERCEL_AUTOMATION_BYPASS_SECRET`, are missing or partially configured.
...
### 5.1 Release proof verified 2026-08-08

Final release SHA: `fb3873ba11853379f792a5ce006656f523a9322d`

| Proof | Run ID | Target | Result |
| --- | ---: | --- | --- |
| Core Funnel staging | [31227583922](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227583922) | `https://vision-board-web-platform-49igew648-anhnhat432s-projects.vercel.app` (`VITE_APP_MODE=demo`) | PASS |
| Email Verification staging | [31227583893](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227583893) | `https://dearourfuture.io.vn` (`real`) | PASS |
| Account Delete staging | [31227583868](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227583868) | `https://dearourfuture.io.vn` (`real`, disposable signup) | PASS |
| LWW staging | [31227583866](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227583866) | `https://dearourfuture.io.vn` (`real`, overwrite-safe account) | PASS |
| Production Smoke #1 | [31227489986](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227489986) | `https://dearourfuture.io.vn` | PASS |
| Production Smoke #2 | [31227851905](https://github.com/anhnhat432/Vision-Board-Web-Platform/actions/runs/31227851905) | `https://dearourfuture.io.vn` | PASS |

The final SHA has a successful Vercel Production deployment (`5803476166`) at `https://vision-board-web-platform-d5zy3983o-anhnhat432s-projects.vercel.app`. The demo proof target was deployed from the exact final tree through deployment commit `ef0e2e4be03e8c57aa39d2b83b0fa4bdce2d1376` at `https://vision-board-web-platform-49igew648-anhnhat432s-projects.vercel.app`. The backend `/api/health` endpoint returns HTTP 200 but does not expose a release commit SHA; Vercel deployment metadata is the source of deployment-version evidence.

Release-proof blockers: None. Separate operational launch controls (backup, monitoring, support, and 24-hour stability) remain tracked in the soft-launch checklist.
...
- End-to-end staging proof for account deletion, signup email verification, and Last-Write-Wins sync passed on the final release SHA in runs `31227583868`, `31227583893`, and `31227583866`; the workflows retain their destructive and overwrite safety guards.
- Manual core-flow launch evidence passed on the protected demo preview in run `31227583922` on the final release SHA. The preview target used the exact final tree and remained in demo mode.
- Account-delete, LWW, email-verification, and protected-preview secret names are ready; `npm run proof:secrets`, `npm run proof:workflows`, and `npm run proof:readiness` all passed on 2026-08-08 without reading secret values or dispatching extra workflows.
