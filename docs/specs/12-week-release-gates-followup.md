...
12. WHEN the expected LWW bootstrap import fails, THE harness SHALL report only same-origin API status plus safe entity counts and client-ID match booleans; it SHALL NOT log request or response bodies, headers, credentials, local snapshots, titles, or secret values.
